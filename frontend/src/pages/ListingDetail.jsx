...
export default function ListingDetail() {
  ...
  const isOwner=user?.id===listing.sellerId, isBuyer=user?.roles?.includes('BUYER')&&!isOwner;
  return <main className="section"><div className="container-wide">
    <Link className="back-link" to="/listings">← Back to marketplace</Link>
    <div className="detail-grid">
      <section>
        <div className="detail-media">{listing.photos?.[0]?<img src={listing.photos[0]} alt={listing.cropType}/>:<div className="media-placeholder">{listing.cropType}</div>}</div>
        <div className="card detail-content"><div className="listing-meta"><span className="tag">AGRICULTURE</span><span className="badge">{listing.status}</span></div><h1>{listing.cropType}</h1><p className="lead">{Number(listing.quantity).toLocaleString()} {listing.unit} available · {listing.location}</p>
        ...
        <p className="muted">MarketBridge is an intermediary. The seller owns the produce and retains price authority.</p></div>
        <div className="card"><h2>Inspection evidence</h2>{listing.inspectionRequests?.length?<>{listing.inspectionRequests.map(r=><div className="evidence" key={r.id}><div><strong>{r.mode.replaceAll('_',' ')}</strong><span className="badge">{r.status}</span></div>{r.inspector&&<p>Inspector: {r.inspector.name}</p>}{r.report?<p>✓ {r.report.quantity} verified · {r.report.grade||'Grade not stated'}{r.report.moisture!=null?` · ${r.report.moisture}% moisture`:''}{r.report.visibleDefects?` · ${r.report.visibleDefects}`:''}</p>:<p className="muted">Report pending.</p>}</div>)}</>:<p className="muted">No inspection yet. Quality-before-payment is the preferred agricultural workflow.</p>}</div>
      </section>
      <aside>
        {msg&&<div className="alert success">{msg}</div>}{error&&<div className="alert error">{error}</div>}
        {isBuyer&&<div className="card sticky-card"><h2>Make an offer</h2><p className="muted">The farmer decides whether to accept, reject or counter.</p><form onSubmit={submitOffer}><label>Your offer (ETB)</label><input required type="number" value={offerAmount} onChange={e=>setOfferAmount(e.target.value)}/><label>Message</label><textarea value={message} onChange={e=>setMessage(e.target.value)} placeholder="Optional message to the farmer"/><button className="btn btn-primary full">Submit offer</button></form><hr/><h3>Quality check</h3><p className="small muted">Request buyer-requested inspection before purchase.</p><button className="btn btn-light full" onClick={()=>requestInspection('BUYER_REQUESTED')}>Request inspection</button></div>}
        {isOwner&&<div className="card sticky-card"><h2>Farmer controls</h2><p className="small muted">Only the farmer can change price or listing status.</p><Link className="btn btn-primary full" to={`/listings/${id}`}>Manage listing</Link><button className="btn btn-light full" onClick={()=>requestInspection('SELLER_REQUESTED')}>Request inspection</button><h3 className="mt">Offers received</h3>{listing.offers?.length?listing.offers.map(o=><OfferRow key={o.id} offer={o} onAction={respondToOffer}/>):<p className="muted">No offers yet.</p>}</div>}
        {!user&&<div className="card"><h2>Ready to participate?</h2><p>Register to make offers, request inspections or list produce.</p><Link className="btn btn-primary full" to="/register">Create account</Link></div>}
      </aside>
    </div>
  </div></main>;
}
